...
struct Vertex 
{
    ...
    Vertex(const glm::vec2& pos, const glm::vec3& color, const glm::vec2& texCoord)
    {
        this->pos = pos;
        this->color = color;
...
public:
    Application()
    {
        ...
        // Vertex vulkan's ndc coord 
        // this->vertices.push_back(Vertex(glm::vec2(-0.5f,  0.5f), glm::vec3(0.0f, 0.0f, 1.0f), glm::vec2(0.0f, 0.0f)));
        // this->vertices.push_back(Vertex(glm::vec2(-0.5f, -0.5f), glm::vec3(1.0f, 1.0f, 1.0f), glm::vec2(0.0f, 1.0f)));
        // this->vertices.push_back(Vertex(glm::vec2( 0.5f, -0.5f), glm::vec3(1.0f, 0.0f, 0.0f), glm::vec2(1.0f, 1.0f)));
        // this->vertices.push_back(Vertex(glm::vec2( 0.5f,  0.5f), glm::vec3(0.0f, 1.0f, 0.0f), glm::vec2(1.0f, 0.0f)));
        // //Index
        ...
        // this->m_bIsNegativeViewport = false;

        // dx/opengl's ndc coord, must specify a negative viewport height to flip it along the y axis
        this->vertices.push_back(Vertex(glm::vec2(-0.5f,  0.5f), glm::vec3(1.0f, 0.0f, 0.0f), glm::vec2(0.0f, 0.0f)));
        this->vertices.push_back(Vertex(glm::vec2(-0.5f, -0.5f), glm::vec3(1.0f, 1.0f, 1.0f), glm::vec2(0.0f, 1.0f)));
        this->vertices.push_back(Vertex(glm::vec2( 0.5f, -0.5f), glm::vec3(0.0f, 0.0f, 1.0f), glm::vec2(1.0f, 1.0f)));
        this->vertices.push_back(Vertex(glm::vec2( 0.5f,  0.5f), glm::vec3(0.0f, 1.0f, 0.0f), glm::vec2(1.0f, 0.0f)));
        this->vertices[0].texCoord.y = 1.0f - this->vertices[0].texCoord.y;
        this->vertices[1].texCoord.y = 1.0f - this->vertices[1].texCoord.y;
        this->vertices[2].texCoord.y = 1.0f - this->vertices[2].texCoord.y;
        this->vertices[3].texCoord.y = 1.0f - this->vertices[3].texCoord.y;
        //Index
        this->indices.push_back(0); this->indices.push_back(1); this->indices.push_back(2);
        this->indices.push_back(2); this->indices.push_back(3); this->indices.push_back(0);
        this->frontFace = VK_FRONT_FACE_CLOCKWISE;
        this->m_bIsNegativeViewport = true;
...
public:
 ...
 private:
    ...
    void initWindow() 
    {
        ...
        this->window = glfwCreateWindow(WIDTH, HEIGHT, "LostPeter - test_vulkan_0025_texture_mapping", nullptr, nullptr);
    ...
    void createInstance()
    {
        ...
        appInfo.pApplicationName = "test_vulkan_0025_texture_mapping";
    ...
    {
        std::string pathVertexShader = GetAssetsPath() + "Assets/Shader/Vulkan/pos2_tex2_color3_ubo.vert.spv";
        std::vector<char> vertShaderCode = readFile(pathVertexShader);
        std::string pathFragmentShader = GetAssetsPath() + "Assets/Shader/Vulkan/pos2_tex2_color3_ubo.frag.spv";
    ...
    {
        size_t count = (int)this->swapChainFramebuffers.size();
        for (size_t i = 0; i < count; i++)
        {
